...
/* ---------- Apartado "Usuario": cambiar foto y guardar datos personales ---------- */
function configurarUsuario() {
    ...
    if (inputFoto) {
        inputFoto.addEventListener('change', () => {
            ...
            lector.onload = (e) => {
                ['userAvatarImg', 'userAvatarImgLg'].forEach(id => {
                    const img = document.getElementById(id);
                    if (!img) return;
                    img.src = e.target.result;
                    img.style.display = 'block';
                });
                ['userAvatarFallback', 'userAvatarFallbackLg'].forEach(id => {
    ...
    if (btnGuardar) {
        btnGuardar.addEventListener('click', () => {
            ...
            if (nombre) {
                document.querySelectorAll('.dash-profile-name').forEach(el => {
                ...
                ['userAvatarFallback', 'userAvatarFallbackLg'].forEach(id => {
                    const span = document.getElementById(id);
                    if (span) span.textContent = iniciales || 'US';
                });
            }
