// mapa.js — Panel / Planea tu Ruta
// Mapa real con Leaflet + OpenStreetMap (gratis, sin API key).
// Requiere conexión a internet: carga teselas del mapa, geocodifica
// direcciones (Nominatim) y calcula la ruta (OSRM, servidor demo público).
...
document.addEventListener('DOMContentLoaded', () => {
    ...
    // Ruta inicial con los valores por defecto del formulario
    calcularRuta();
});
...
function inicializarMapa() {
    ...
    // Asegura que Leaflet calcule bien el tamaño tras el primer render
    setTimeout(() => mapa.invalidateSize(), 200);
}

/* ---------- Líneas del Metro (trazado aproximado, referencial) ---------- */
...
/* ---------- Geocodificar un lugar dentro de Medellín (Nominatim) ---------- */
...
/* ---------- Trazar la ruta real entre dos puntos (OSRM) ---------- */
async function trazarRuta(origenGeo, destinoGeo) {
    ...
    // El costo se mantiene fijo: el Metro de Medellín usa tarifa plana,
    // no varía según la distancia recorrida.
}

/* ---------- Flujo completo: buscar origen/destino y trazar ruta ---------- */
async function calcularRuta() {
    ...
    try {
        ...
        console.error(err);
        mostrarToast('No se pudo calcular la ruta. Revisa tu conexión e intenta de nuevo.');
    } finally {
