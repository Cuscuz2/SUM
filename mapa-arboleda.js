...
document.addEventListener('DOMContentLoaded', () => {
    ...
    configurarCierreAvisoTransbordo();
...
const LINEAS = {
    ...
    k:       { estaciones: ESTACIONES_K,       color: '#0d9488', dash: '6 4',    categoria: 'metrocable' },
    l:       { estaciones: ESTACIONES_L,       color: '#0d9488', dash: '6 4',    categoria: 'metrocable' },
    h:       { estaciones: ESTACIONES_H,       color: '#0d9488', dash: '6 4',    categoria: 'metrocable' },
    j:       { estaciones: ESTACIONES_J,       color: '#0d9488', dash: '6 4',    categoria: 'metrocable' },
    ...
    tranvia: { estaciones: ESTACIONES_TRANVIA, color: '#7c3aed', dash: '6 4',    categoria: 'tranvia' }
};
...
function dibujarRedCompleta() {
    try {
        ...
        Object.values(LINEAS).forEach(linea => {
            const puntos = linea.estaciones.map(e => [e.lat, e.lon]);

            // Contorno blanco (casing) debajo de cada línea para que resalte sobre el
            // basemap y no se confunda con otras líneas que comparten tramo (p. ej.
            // Metroplús vs. Metro Línea A en el centro).
            const casing = L.polyline(puntos, {
                color: '#ffffff',
                weight: linea.dash ? 7 : 6,
                opacity: 0.9,
                lineCap: 'round',
                lineJoin: 'round'
            });
            ...
            gruposPorCategoria[linea.categoria].push(casing, polilinea, ...marcadores);
        });
        ...
        // Metrocable y Metroplús comparten estaciones con el Metro (Industriales,
        // Hospital, Cisneros, Prado...); los traemos al frente para que no queden
        // tapados por la línea A/B. bringToFront() solo existe en capas
        // vectoriales individuales (Polyline/CircleMarker), NO en un LayerGroup,
        // así que hay que recorrer las capas hijas una por una.
        ['tranvia', 'metrocable', 'metroplus'].forEach(cat => {
            const grupo = capasMetro[cat];
            if (!grupo) return;
            grupo.eachLayer(capa => {
                if (typeof capa.bringToFront === 'function') capa.bringToFront();
            });
        });
    } catch (err) {
        console.error('Error dibujando la red del sistema:', err);
    }
}
...
function configurarLeyenda() {
    ...
    // Fallback defensivo: si el CSS del contenedor no permite salto de línea
    // o recorta el desbordamiento, esto evita que las últimas píldoras
    // (Metrocable, Metroplús) queden invisibles al agregar más categorías.
...
function configurarCierreAvisoTransbordo() {
    const boton = document.getElementById('transferAlertClose');
    const alerta = document.getElementById('transferAlert');
    if (!boton || !alerta) return;
    boton.addEventListener('click', () => { alerta.hidden = true; });
}
...
async function calcularRuta() {
    ...
    const { total, usaArvi } = calcularTarifaViaje(resultado.lineas);
    document.getElementById('summaryCost').textContent = formatearCOP(total);
    ...
    mostrarAvisoTransbordo(transbordos);
    btnNavigate.disabled = false;
}

/* ---------- Aviso de tiempo límite para hacer el transbordo ---------- */
function mostrarAvisoTransbordo(transbordos) {
    const alerta = document.getElementById('transferAlert');
    const texto = document.getElementById('transferAlertText');
    if (!alerta || !texto) return;

    if (transbordos > 0) {
        const plural = transbordos > 1;
        texto.textContent = `Tu ruta tiene ${transbordos} transbordo${plural ? 's' : ''}: tienes 90 minutos desde que pagas tu tarifa para ingresar al siguiente transporte, de lo contrario se te cobrará nuevamente.`;
        alerta.hidden = false;
    } else {
        alerta.hidden = true;
    }
